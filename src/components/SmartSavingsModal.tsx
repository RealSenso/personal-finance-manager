...
export const SmartSavingsModal: React.FC<SmartSavingsModalProps> = ({
  ...
  const [strategy, setStrategy] = useState<SavingsStrategy>('balanced_accelerator');
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-150 max-h-[92vh] flex flex-col">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400">
                Calculates monthly deposits to buy your items as early as possible while still depositing into everything.
              </p>
            </div>
          </div>
          <button
          ...
          <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-xs text-zinc-400 font-mono">/ month available</span>
              </div>
              <div className="text-[11px] text-zinc-400 font-mono">
                {formatCurrency(result.totalMonthlyIncome)} income - {formatCurrency(result.totalRecurringExpenses)} recurring expenses
              </div>
            </div>
            ...
            {result.cascadeTimelineSummary && (
              <div className="bg-zinc-950/80 border border-zinc-800 rounded-lg p-3 text-xs space-y-1 sm:max-w-xs">
                <div className="text-zinc-200 font-medium flex items-center gap-1.5">
                  ...
                  <span>Earliest Purchase in {result.cascadeTimelineSummary.firstGoalReadyInMonths} mos</span>
                </div>
                <div className="text-[11px] text-zinc-400">
                  <strong className="text-zinc-300 font-semibold">{result.cascadeTimelineSummary.firstGoalName}</strong> will be fully funded first, then its monthly deposit cascades to your other goals!
                </div>
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-zinc-200">Balanced Accelerator</span>
                  {strategy === 'balanced_accelerator' && (
                    <Check className="w-3.5 h-3.5 text-emerald-400" />
                  )}
                </div>
                <p className="text-[11px] text-zinc-400 mt-1">
                  Guaranteed baseline deposit into every goal + 70% surplus accelerated to buy nearest item fastest.
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-zinc-200">Maximum Velocity</span>
                  {strategy === 'high_velocity' && (
                    <Check className="w-3.5 h-3.5 text-indigo-400" />
                  )}
                </div>
                <p className="text-[11px] text-zinc-400 mt-1">
                  Small token baseline deposit to other items + 85% focused on buying the closest item ASAP.
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-zinc-200">Even Progress</span>
                  {strategy === 'proportional' && (
                    <Check className="w-3.5 h-3.5 text-zinc-200" />
                  )}
                </div>
                <p className="text-[11px] text-zinc-400 mt-1">
                  50% equal baseline + 50% target weighting. All items progress at similar percentage speeds.
                </p>
              </button>
            </div>
          </div>
              ...
              {result.allocations.map((alloc) => {
                const percentDone =
                  alloc.targetAmount > 0
                    ? Math.min(100, Math.round((alloc.currentBalance / alloc.targetAmount) * 100))
                    : 100;
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-[200px]">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <h4 className="text-sm font-bold text-zinc-100">{alloc.bucketName}</h4>
                        {alloc.isAchieved && (
                      ...
                      <div className="mt-2 flex items-center justify-between text-xs font-mono text-zinc-400">
                        <span>
                          {formatCurrency(alloc.currentBalance)} of {formatCurrency(alloc.targetAmount)}
                        </span>
                      ...
                      <div className="text-[11px] font-mono text-zinc-400 mt-1">
                        {alloc.remainingAmount > 0 ? (
                          <span>{formatCurrency(alloc.remainingAmount)} remaining</span>
                        ) : (
                    ...
                    <div className="sm:border-l sm:border-zinc-800 sm:pl-4 min-w-[240px]">
                      ...
                      <div className="text-xl font-extrabold font-mono text-white mt-0.5 flex items-baseline gap-2">
                        <span>{formatCurrency(alloc.recommendedMonthly)}/mo</span>
                      ...
                      {!alloc.isAchieved && (
                        <div className="flex items-center gap-1.5 text-[10px] font-mono text-zinc-400 mt-1">
                          <span className="text-zinc-300">₹{alloc.baselineShare.toLocaleString('en-IN')} base</span>
                          <span>+</span>
                          <span className="text-emerald-400 font-medium">₹{alloc.acceleratorShare.toLocaleString('en-IN')} booster</span>
                        </div>
                      )}
                      ...
                      {alloc.monthsSavedVsEqualSplit > 0 && (
                        <div className="text-[10px] text-emerald-400/90 font-medium mt-0.5">
                          ⚡ Ready ~{alloc.monthsSavedVsEqualSplit} mos earlier than equal split
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-emerald-950/20 border border-emerald-500/20 rounded-xl p-3.5 text-xs text-emerald-300/90 flex items-start gap-2.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-emerald-400/80 leading-relaxed">
                By accelerating the closest item, you get to buy it much earlier. The exact moment it is fully funded, its entire monthly allocation automatically cascades to turbocharge your next savings goal!
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-zinc-800 flex items-center justify-between bg-zinc-950">
          <div className="text-xs text-zinc-400">
            Total Allocated: <strong className="text-white font-mono">{formatCurrency(result.allocations.reduce((s, a) => s + a.recommendedMonthly, 0))}</strong> / {formatCurrency(result.availableSavingsPool)}
          </div>
