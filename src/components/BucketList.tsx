...
export const BucketList: React.FC<BucketListProps> = ({
  ...
  const pill = (v: typeof filterType, label: string, icon?: React.ReactNode) => (
    <button
      ...
      className={`px-2.5 py-1 rounded-md text-xs font-display font-medium tracking-wide transition-colors cursor-pointer flex items-center gap-1 ${
        filterType === v ? 'bg-emerald-500 text-zinc-950' : 'text-zinc-400 hover:text-zinc-100'
      }`}
    >
      {icon}
      {label}
  ...
  return (
    <div className="bl-panel bl-cut bg-zinc-900/40 border border-zinc-800 rounded-2xl flex flex-col min-h-0 h-full">
      {/* Header */}
      <div className="shrink-0 flex flex-col gap-2.5 p-3 border-b border-zinc-800/80">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <span className="bl-label text-emerald-400 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-2">
            {savingsCount > 0 && onOpenSmartSavings && (
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 bl-cut text-xs font-semibold text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/20 transition-colors cursor-pointer"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 bl-cut text-xs font-medium text-zinc-200 bg-zinc-900 border border-zinc-700/80 hover:border-emerald-500/40 hover:text-white transition-colors cursor-pointer"
            >
        ...
        <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-lg p-0.5 self-start">
          ...
          {pill('recurring', `Recurring ${recurringCount}`, <Wallet className="w-3 h-3 text-cyan-400" />)}
          {pill('savings_goal', `Goals ${savingsCount}`, <PiggyBank className="w-3 h-3 text-emerald-400" />)}
        </div>
      </div>

      {/* Scroll region */}
      <div className="bl-scroll p-3 grid grid-cols-1 xl:grid-cols-2 gap-3 content-start">
        {filtered.length === 0 ? (
          ...
          filtered.map((bucket, i) => (
            <div key={bucket.id} className="anim-rise" style={{ animationDelay: `${i * 35}ms` }}>
              <BucketCard
                bucket={bucket}
