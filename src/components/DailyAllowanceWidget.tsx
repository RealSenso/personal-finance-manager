...
import {
  ...
  simulateTomorrowAllowance,
} from "../lib/dailyAllowance";
...
export const DailyAllowanceWidget: React.FC<DailyAllowanceWidgetProps> = ({
  ...
  return (
    <div className="m-panel shrink-0 bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-950 border border-emerald-500/25 rounded-2xl p-4 shadow-lg relative overflow-hidden">
        ...
        <div className="flex items-start sm:items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="m-label text-emerald-400">Safe Daily Spend</span>
              ...
              <span
                className={`text-[10px] font-display uppercase tracking-wider ${paceTone}`}
              >
                {paceLabel}
              </span>
            </div>
            ...
            <div className="flex items-baseline gap-2.5 mt-1">
              <span className="font-display text-3xl sm:text-4xl font-bold text-zinc-100 tracking-tight tabular-nums">
            ...
            <p className="text-xs text-zinc-400 mt-1">
              {formatCurrency(allowance.remainingSpendable)} left this month
              after {formatCurrency(allowance.savingsReserve)} savings +{" "}
        ...
        <div className="mt-4 pt-4 border-t border-zinc-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-start gap-2.5">
            ...
            <p className="text-xs text-zinc-300">
              ...
              under your steady pace of{" "}
              {formatCurrency(allowance.baselineDaily)}/day so far. Move it to
              your goals before it drifts back into spending.
            </p>
          </div>
          {onSweepToGoals && (
            <button
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-zinc-400 mt-0.5">
                  How spending a set amount today changes tomorrow's safe
                  allowance.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative w-28">
                  <span className="absolute left-2.5 top-1.5 text-xs text-zinc-500 font-mono">
                    ₹
                  </span>
                  <input
                    type="number"
                    ...
                    className="w-full bg-zinc-900 border border-zinc-700 rounded-lg pl-6 pr-2 py-1 text-xs text-zinc-100 font-mono focus:outline-none focus:border-emerald-500"
                    placeholder="500"
                  />
                </div>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs pt-3 border-t border-zinc-800/60 font-mono">
              <div className="p-2.5 rounded-lg bg-zinc-900/90 border border-zinc-800">
                ...
                <span className="text-zinc-100 font-bold text-sm">
                  {formatCurrency(
                    Math.max(0, allowance.safeDailyAllowance - numSimulated),
                  )}
                </span>
              </div>
              ...
              <div className="p-2.5 rounded-lg bg-zinc-900/90 border border-zinc-800">
                <span className="text-zinc-400 block text-[10px]">
                  Month-End Spendable Left
                </span>
                <span className="text-zinc-200 font-bold text-sm">
                  {formatCurrency(
                    Math.max(0, allowance.remainingSpendable - numSimulated),
                  )}
                </span>
              </div>
            </div>
