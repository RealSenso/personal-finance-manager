...
const Plate: React.FC<{
  ...
  const bar =
    tone === 'rose' ? 'from-rose-500' : tone === 'cyan' ? 'from-cyan-400' : 'from-emerald-500';
  return (
    <div className="bl-panel bl-cut relative overflow-hidden bg-zinc-900/70 border border-zinc-800 rounded-xl p-3.5 hover:border-emerald-500/40 transition-colors">
      <div className={`absolute left-0 top-0 h-full w-[3px] bg-gradient-to-b ${bar} to-transparent`} />
      <div className="flex items-center justify-between">
        <span className="bl-label flex items-center gap-1.5">
          {icon}
          {label}
        </span>
      </div>
      <div className="mt-1.5 font-display text-2xl font-bold text-white tracking-tight tabular-nums">
        {value}
      </div>
      {sub && <div className="text-[11px] text-zinc-400 font-mono mt-0.5">{sub}</div>}
      {foot && (
...
export const DashboardStats: React.FC<DashboardStatsProps> = ({
  ...
  const currentDay = isCurr ? Math.min(today.getDate(), totalDaysInMonth) : totalDaysInMonth;
  ...
  const goals = buckets.filter((b) => b.type === 'savings_goal' && !b.isArchived);
  ...
  const depositedThisMonth = transactions
    .filter((t) => t.type === 'savings_deposit' && t.date.startsWith(currentMonth))
    .reduce((s, t) => s + t.amount, 0);
  ...
  return (
    <div className="bl-panel bl-cut bg-zinc-900/40 border border-zinc-800 rounded-2xl p-3 flex flex-col min-h-0">
      <div className="flex items-center justify-between px-1 pb-2">
        <span className="bl-label text-emerald-400">Player Stats</span>
        <button
      ...
      <div className="bl-scroll pr-1 flex flex-col gap-3">
          ...
          <Plate
            ...
            foot={
              <>
                ...
                <span className="font-mono text-emerald-300">{da.savingsRatePercent}%</span>
              </>
            }
          />
          {editing && (
            <div className="absolute top-2 left-2 right-2 z-30 bg-zinc-950 border border-emerald-500/40 rounded-xl p-3.5 shadow-2xl glow-blue">
              <form onSubmit={saveIncome} className="space-y-2.5">
                <div className="bl-label text-emerald-400">Adjust</div>
                {[
                  ...
                  <div key={lbl as string}>
                    <label className="text-[11px] text-zinc-400 block mb-1">{lbl as string}</label>
                    <input
                      type="number"
                      value={val as string}
                      onChange={(e) => (set as (v: string) => void)(e.target.value)}
                      className="w-full bg-zinc-900 border border-zinc-700 rounded px-2.5 py-1.5 text-xs text-white font-mono focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                ))}
        ...
        <Plate
          ...
          foot={
            <>
              <span title={`${formatCurrency(da.savingsReserve)} savings + ${formatCurrency(da.goalCommitments)} goals + ${formatCurrency(da.fixedCommitments)} bills`}>
                Reserved
              </span>
              <span className="font-mono text-indigo-400">{formatCurrency(da.totalReserved)}</span>
            </>
          }
        />
        ...
        <Plate
          ...
          foot={
            <>
              <span>Safe / day</span>
              <span className="font-mono text-emerald-300">{formatCurrency(da.safeDailyAllowance)}</span>
            </>
          }
        />
        ...
        <Plate
          ...
          foot={
            <>
              ...
              <span className="font-mono text-cyan-400">+{formatCurrency(depositedThisMonth)}</span>
            </>
          }
        />

        {/* Month timeline */}
        <div className="bl-panel bl-cut bg-zinc-900/70 border border-zinc-800 rounded-xl p-3">
          <div className="flex items-center justify-between bl-label">
            <span>Day {currentDay} / {totalDaysInMonth}</span>
            <span className="text-emerald-400">{monthPct}%</span>
          </div>
          <div className="mt-2 h-2 bg-zinc-800 rounded-full overflow-hidden relative anim-sweep">
            <div
