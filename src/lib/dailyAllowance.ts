...
export function calculateDailyAllowance(
  ...
  const savingsRatePercent = Math.min(
    90,
    Math.max(0, incomeProfile.savingsRatePercent ?? 25),
  );
  ...
  const goalDepositsPlanned = savingsGoals.reduce(
...
export function simulateTomorrowAllowance(
  ...
  const remainingAfterSpend = Math.max(
    0,
    remainingSpendable - hypotheticalSpendToday,
  );
